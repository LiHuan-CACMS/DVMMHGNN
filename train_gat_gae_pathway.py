...
PATHWAY_XLSX = os.path.join(OUTDIR, "Pathway.norm.xlsx")        
EMB_PT       = os.path.join(OUTDIR, "emb_pathway_text.pt")       
EDGE_CSV     = os.path.join(OUTDIR, "pathway_edges_topk_undirected.csv")  
...
def load_graph():
    ...
    edges = edges[edges["source"].isin(name2id) & edges["target"].isin(name2id)]
    src = edges["source"].map(name2id).to_numpy()
    tgt = edges["target"].map(name2id).to_numpy()
...
def train_loop(
    ...
    device=None, outdir="process", names=None
):
    ...
    torch.save(z_all, os.path.join(outdir, "emb_pathway_gat_gae.pt"))
    out_csv = os.path.join(outdir, "emb_pathway_gat_gae.csv")
...
if __name__ == "__main__":
    ...
    model, z_all, metrics = train_loop(
        ...
        device=device, outdir=OUTDIR, names=names
    )
